...
class SafetyService {
   ...
   */
  private async getEmergencyServices(lat: number, lng: number, radius: number): Promise<SafetyData[]> {
    const query = `
      [out:json][timeout:25];
      (
    ...
    try {
      ...
      return data.elements.map((element: any) => ({
        ...
        title: element.tags?.name || this.getDefaultName(element.tags?.amenity || element.tags?.emergency),
        ...
        address: element.tags?.['addr:full'] || `${element.tags?.['addr:housenumber'] || ''} ${element.tags?.['addr:street'] || ''}`.trim()
      }));
    } catch (error) {
   ...
   * Get community safety reports
   */
  private async getCommunityReports(lat: number, lng: number, radius: number): Promise<SafetyData[]> {
    ...
    const communityReports = [
      ...
      },
      {
        ...
        description: 'Community reports suggest avoiding this area after sunset due to poor lighting.',
  ...
  async getSocialAlerts(lat: number, lng: number, radius: number = 5000): Promise<SocialAlert[]> {
    ...
    const alertTemplates = [
      ...
      },
      {
        ...
        content: 'PSA: Increased police presence in downtown area due to event. All good, just FYI.',
        urgency: 'info' as const,
        engagement: 23
      },
      {
        platform: 'local_news' as const,
        content: 'Local authorities advise caution due to construction work affecting pedestrian routes.',
        urgency: 'warning' as const,
        engagement: 67
      },
      {
  ...
  async submitSafetyReport(report: {
    ...
    try {
      ...
      return {
        ...
        message: 'Thank you for helping keep our community safe! Your report has been submitted for review.'
      };
    } catch (error) {
      return {
        success: false,
